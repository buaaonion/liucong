...
public class ConditionTermENode extends BNode {
	...
	public void typeCheck() throws SemanticsException {
		...
		if (getChildNodes().size() == 1) {
			...
			if (bNode instanceof MSENode) {
				bNode.typeCheck();
				this.setOpe = ((MSENode) bNode).isSetOpe();
				this.type = ((MSENode) bNode).getType();
			} else {
				bNode.typeCheck();
	...
	public void toCCode() {
		...
		if (getChildNodes().size() == 1) {
			BNode bNode = getChildNodeAt(0);
			if (bNode instanceof MSENode) {
				((MSENode) bNode).setLeftType(this.leftType);
			}
			bNode.toCCode();
